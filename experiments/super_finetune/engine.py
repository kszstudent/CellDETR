...
class trainer():
    def __init__(self, args:dict,wandb_log:wandb):
        ...
        if getattr(self.args.experiment, 'freeze_shared', False):
            # freeze shared backbone + transformer, only train decouple-specific heads
            for n, p in self.model.named_parameters():
                if n.startswith('detr.'):
                    p.requires_grad_(False)
            print("Shared parts (detr.*) frozen.")
        param_dicts = [
