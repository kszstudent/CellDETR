...
def build_resnet_backbone(cfg):
    ...
    frozen_layers = []
    if cfg.backbone.frozen_stages > 0:
        frozen_layers = list(range(1, cfg.backbone.frozen_stages + 1))

    # normalization layer to be used (FrozenBatchNorm2d or nn.BatchNorm2d)
    norm_layer = FrozenBatchNorm2d if cfg.backbone.frozen_bn else nn.BatchNorm2d
    
    backbone = ResNetBackbone(
        name=cfg.backbone.name,
        train_backbone=train_backbone,
        frozen_layers=frozen_layers,
        return_layers=cfg.backbone.return_layers,
